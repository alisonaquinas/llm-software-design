---
name: haskell-docstrings
description: >
  document haskell code with haddock comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain haskell source documentation, inline api help, or machine-readable comments.
---

# Haskell Docstrings

Use this skill to add or normalize Haddock comments in Haskell code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on Haddock comments for the requested Haskell surface.
...
```text
Add or normalize Haskell source documentation for this public API without changing behavior.
```

```text
Review this Haskell file for missing or misleading machine-readable documentation that external tools depend on.
```
