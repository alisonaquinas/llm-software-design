...
- Adopt strict types, explicit visibility, and typed signatures at public boundaries wherever the runtime and framework permit.
- Follow PSR conventions for layout, autoloading, and interface contracts so libraries compose predictably.
- Prefer dependency injection, value objects, and clear request boundaries over global state and service locators.
- Treat input validation, output escaping, and database parameterization as mandatory, not optional hardening.
- Automate formatting, static analysis, and unit tests in the normal development loop.
