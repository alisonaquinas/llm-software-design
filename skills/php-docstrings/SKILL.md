...
name: php-docstrings
description: >
  document php code with phpdoc comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain php source documentation, inline api help, or machine-readable comments.
---

# PHP Docstrings

Use this skill to add or normalize PHPDoc comments in PHP code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on PHPDoc comments for the requested PHP surface.
...
```text
Add or normalize PHP source documentation for this public API without changing behavior.
```

```text
Review this PHP file for missing or misleading machine-readable documentation that external tools depend on.
```
