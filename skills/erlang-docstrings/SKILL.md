---
name: erlang-docstrings
description: >
  document erlang code with edoc comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain erlang source documentation, inline api help, or machine-readable comments.
---

# Erlang Docstrings

Use this skill to add or normalize EDoc comments in Erlang code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on EDoc comments for the requested Erlang surface.
...
```text
Add or normalize Erlang source documentation for this public API without changing behavior.
```

```text
Review this Erlang file for missing or misleading machine-readable documentation that external tools depend on.
```
