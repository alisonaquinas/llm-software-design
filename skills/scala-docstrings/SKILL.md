---
name: scala-docstrings
description: >
  document scala code with scaladoc comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain scala source documentation, inline api help, or machine-readable comments.
---

# Scala Docstrings

Use this skill to add or normalize Scaladoc comments in Scala code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on Scaladoc comments for the requested Scala surface.
...
```text
Add or normalize Scala source documentation for this public API without changing behavior.
```

```text
Review this Scala file for missing or misleading machine-readable documentation that external tools depend on.
```
