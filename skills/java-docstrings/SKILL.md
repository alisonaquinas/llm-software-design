---
name: java-docstrings
description: >
  document java code with javadoc comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain java source documentation, inline api help, or machine-readable comments.
---

# Java Docstrings

Use this skill to add or normalize Javadoc comments in Java code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on Javadoc comments for the requested Java surface.
...
```text
Add or normalize Java source documentation for this public API without changing behavior.
```

```text
Review this Java file for missing or misleading machine-readable documentation that external tools depend on.
```
