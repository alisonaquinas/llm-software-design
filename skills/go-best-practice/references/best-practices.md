...
- Let `gofmt` define layout and keep names short but meaningful within a narrow scope.
- Prefer simple packages, small interfaces owned by the consumer, and explicit error returns.
- Use `context.Context` for cancellation, deadlines, and request-scoped data only.
- Design concurrency so goroutine ownership, shutdown, and backpressure are obvious from the call graph.
- Write table-driven tests for boundary conditions and keep `go test`, vetting, and race checks easy to run.
