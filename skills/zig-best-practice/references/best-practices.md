...
- Make allocator ownership explicit and thread it through APIs rather than hiding memory policy in globals.
- Use error unions, optionals, and compile-time checks to express failure and configuration clearly.
- Prefer straightforward control flow and explicit resource cleanup over clever abstraction.
- Keep unsafe or target-specific code small and near the boundary that requires it.
- Use `zig fmt`, built-in tests, and cross-target builds as routine validation.
...
- [The Zen of Zig](https://ziglang.org/documentation/0.15.2/#Zen)
