---
name: ruby-docstrings
description: >
  document ruby code with yard comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain ruby source documentation, inline api help, or machine-readable comments.
---

# Ruby Docstrings

Use this skill to add or normalize YARD comments in Ruby code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on YARD comments for the requested Ruby surface.
...
```text
Add or normalize Ruby source documentation for this public API without changing behavior.
```

```text
Review this Ruby file for missing or misleading machine-readable documentation that external tools depend on.
```
