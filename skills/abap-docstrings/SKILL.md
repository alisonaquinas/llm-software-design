---
name: abap-docstrings
description: >
  document abap code with abap doc comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain abap source documentation, inline api help, or machine-readable comments.
---

# ABAP Docstrings

Use this skill to add or normalize ABAP Doc comments in ABAP code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on ABAP Doc comments for the requested ABAP surface.
...
```text
Add or normalize ABAP source documentation for this public API without changing behavior.
```

```text
Review this ABAP file for missing or misleading machine-readable documentation that external tools depend on.
```
