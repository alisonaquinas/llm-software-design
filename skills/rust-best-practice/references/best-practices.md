...
- Use the type system to encode invariants: ownership, borrowing, lifetimes, and enum states should clarify design rather than fight it.
- Prefer `Result` and `Option`-based APIs, and reserve panics for impossible states or test-only code.
- Keep `unsafe` blocks small, documented, and surrounded by safe abstractions and tests.
- Favor explicit modules, newtypes, and iterator-based transformations when they improve intent and error handling.
- Run `rustfmt`, `clippy`, and tests as standard gates for every change.
