...
- Confirm the dialect first; Common Lisp, Scheme, and other Lisps differ enough that advice must match the runtime.
- Prefer small pure functions and clear data flow before reaching for macros or dynamic rebinding.
- Use macros to remove boilerplate or encode domain forms, not to hide ordinary control flow.
- Keep package or namespace boundaries explicit and avoid accidental symbol leakage.
- Document reader, macro, and special-variable conventions because they are easy for new maintainers to misread.
