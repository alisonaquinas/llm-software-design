...
- Optimize for readability by young or occasional programmers: clear sprite names, short scripts, and obvious broadcast names.
- Separate setup, game loop, input handling, and scoring logic into visually distinct blocks or custom blocks.
- Reduce duplicated block stacks by extracting repeated behavior into custom blocks when the audience can still follow the flow.
- Use comments, backdrop changes, and naming conventions to explain scene transitions and shared state.
- Test interactive edge cases such as restart behavior, clone cleanup, and message ordering.
...
- [Scratch Style Guide](https://en.scratch-wiki.info/wiki/Scratch_Style_Guide)
