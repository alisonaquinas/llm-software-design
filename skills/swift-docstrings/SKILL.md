---
name: swift-docstrings
description: >
  document swift code with swift markup comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain swift source documentation, inline api help, or machine-readable comments.
---

# Swift Docstrings

Use this skill to add or normalize Swift Markup comments in Swift code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on Swift Markup comments for the requested Swift surface.
...
```text
Add or normalize Swift source documentation for this public API without changing behavior.
```

```text
Review this Swift file for missing or misleading machine-readable documentation that external tools depend on.
```
