...
- Lean on OTP behaviors, supervision trees, and process isolation instead of inventing custom lifecycle patterns.
- Keep message protocols explicit and document mailbox expectations, timeouts, and restart semantics.
- Use pattern matching and function clauses for clarity, but avoid clever branching that hides state transitions.
- Separate pure transformation logic from process orchestration so tests stay fast and deterministic.
- Run formatting, dialyzer, and tests routinely across supported OTP versions.
...
- [Erlang Reference Manual](https://www.erlang.org/docs)
- [Erlang System Principles](https://www.erlang.org/docs/24/system_principles/users_guide)
