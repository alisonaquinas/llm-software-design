...
name: sas-docstrings
description: >
  document sas code with structured header comments above macros and steps so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain sas source documentation, inline api help, or machine-readable comments.
---

# SAS Docstrings

Use this skill to add or normalize structured header comments above macros and steps in SAS code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on structured header comments above macros and steps for the requested SAS surface.
...
```text
Add or normalize SAS source documentation for this public API without changing behavior.
```

```text
Review this SAS file for missing or misleading machine-readable documentation that external tools depend on.
```
