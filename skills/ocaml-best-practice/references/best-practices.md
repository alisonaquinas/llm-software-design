...
- Use modules, signatures, and algebraic data types to make domain structure and encapsulation obvious.
- Prefer immutable data and total pattern matches where practical; localize exceptions and mutation to necessary edges.
- Keep interface files intentional so public contracts stay smaller than implementation details.
- Adopt one formatter profile and one build layout with `dune` so structure does not drift across packages.
- Test parsing, serialization, and stateful boundaries explicitly because the type system cannot prove everything.
...
- [ocamlformat](https://ocaml.org/p/ocamlformat/latest/doc/README.html)
