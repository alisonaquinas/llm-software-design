...
- Write queries for readability first: explicit column lists, stable aliasing, meaningful names, and one consistent layout.
- Treat dialect differences as design constraints; identify the target engine before recommending syntax or indexing patterns.
- Prefer set-based operations, explicit transactions, and parameterized statements over string-built SQL and row-by-row loops.
- Model integrity in the schema with keys, constraints, and defaults instead of pushing all validation into application code.
- Benchmark query plans and I/O characteristics before recommending denormalization or hint-heavy tuning.
