...
- Default to local scope, small modules, and explicit return tables rather than ambient globals.
- Keep metatables and dynamic tricks for places where they clearly simplify the design.
- Document host-runtime assumptions because Lua often lives inside a larger product with nonstandard libraries.
- Treat `nil` semantics and table shape carefully, especially at API boundaries and serialization edges.
- Use formatter, linter, and test tooling appropriate to the host environment when available.
