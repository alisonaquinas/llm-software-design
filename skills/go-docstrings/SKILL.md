...
name: go-docstrings
description: >
  document go code with go doc comments so external tools can read the embedded api documentation. use when the request is to add, normalize, migrate, review, or explain go source documentation, inline api help, or machine-readable comments.
---

# Go Docstrings

Use this skill to add or normalize Go doc comments in Go code so external tools can discover API intent without reverse-engineering the implementation.
...
3. Otherwise standardize on Go doc comments for the requested Go surface.
...
```text
Add or normalize Go source documentation for this public API without changing behavior.
```

```text
Review this Go file for missing or misleading machine-readable documentation that external tools depend on.
```
